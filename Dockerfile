...
RUN --mount=type=cache,target=/root/.cache CMAKE_ARGS="-DLLAMA_CUBLAS=on" FORCE_CMAKE=1 pip install --upgrade --force-reinstall llama-cpp-python==0.1.78

#RUN --mount=type=cache,target=~/.cache CMAKE_ARGS="-DLLAMA_CUBLAS=on" FORCE_CMAKE=1 pip install --upgrade --force-reinstall llama-cpp-python
...
ARG device_type=cpu
RUN --mount=type=cache,target=~/.cache python ingest.py --device_type $device_type
COPY . .
